...
public class DefaultJavaParser implements Parser<CompilationUnit> {
    ...
    public CompilationUnit parse(String text, boolean withoutLocation) throws ParseException {
        try {
    ...
    @Override
    public CompilationUnit parse(File file, String encoding) throws ParseException {
        try {
            return ASTManager.parse(file, encoding);
        } catch (Exception e) {
            throw new ParseException(e.getCause());
